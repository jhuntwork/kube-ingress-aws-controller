...
func (a *Adapter) newIngress(typ IngressType, metadata kubeItemMetadata, host string, hostnames []string) (*Ingress, error) {
	...
	cascade := true
	if getAnnotationsString(annotations, ingressNLBCascadeAnnotation, "false") == "false" {
		cascade = false
	}
	if cascade && loadBalancerType != loadBalancerTypeNLB {
		return nil, errors.New("cascading is only supported on NLBs")
	}
