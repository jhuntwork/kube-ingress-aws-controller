...
func createStack(awsAdapter *aws.Adapter, lb *loadBalancer, problems *problem.List) {
	...
	stackId, err := awsAdapter.CreateStack(certificates, lb.scheme, lb.securityGroup, lb.Owner(), lb.sslPolicy, lb.ipAddressType, lb.wafWebACLID, lb.cwAlarms, lb.loadBalancerType, lb.http2, lb.extraListeners, lb.cascade)
	if err != nil {
...
func updateStack(awsAdapter *aws.Adapter, lb *loadBalancer, problems *problem.List) {
	...
	stackId, err := awsAdapter.UpdateStack(lb.stack.Name, certificates, lb.scheme, lb.securityGroup, lb.Owner(), lb.sslPolicy, lb.ipAddressType, lb.wafWebACLID, lb.cwAlarms, lb.loadBalancerType, lb.http2, lb.extraListeners, lb.cascade)
...
func cniEventHandler(ctx context.Context, targetCNIcfg *aws.TargetCNIconfig,
	...
	for {
		select {
		case <-ctx.Done():
			log.Debugln("received a cancel signal, eventHandler exiting")
			return
